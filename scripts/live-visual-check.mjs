...
try {
  ...
  if(!workMetrics)throw new Error('Work visual QA could not find the unified queue.'); if(workMetrics.tabs.length!==3||!normalizedTabs.some(x=>x?.startsWith('to do'))||!normalizedTabs.some(x=>x?.startsWith('in progress'))||!normalizedTabs.some(x=>x?.startsWith('done')))throw new Error('Work queue must expose To Do, In Progress, and Done.'); if(!workMetrics.hasIncident||!workMetrics.hasChanges||!workMetrics.hasNotes)throw new Error('Work must expose Incidents, Changes, and Notes as separate sections.'); if(workMetrics.hasHorizontalOverflow)throw new Error('Work page has horizontal overflow at the mobile viewport.'); await capture(cdp,'work-mobile');

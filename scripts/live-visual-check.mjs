...
try {
  ...
  const workUrl=new URL('work',liveUrl).toString(); await evaluate(cdp,`window.history.pushState({},'',${JSON.stringify(workUrl)});window.dispatchEvent(new PopStateEvent('popstate'));true`); await poll(cdp,"Boolean(document.querySelector('.work-queue-page'))",'unified Work queue'); await new Promise(r=>setTimeout(r,600));
  const workMetrics=await evaluate(cdp,`(()=>{const page=document.querySelector('.work-queue-page');const tabs=[...document.querySelectorAll('.wq-tabs button')].map(x=>x.textContent);const headings=[...document.querySelectorAll('.wq-section h2')].map(x=>x.textContent);if(!page)return null;return{width:page.getBoundingClientRect().width,tabs,headings,hasIncident:headings.includes('Incidents'),hasChanges:headings.includes('Changes'),hasNotes:headings.includes('Notes'),hasHorizontalOverflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+1}})()`);
  if(!workMetrics)throw new Error('Work visual QA could not find the unified queue.'); if(workMetrics.tabs.length!==3||!workMetrics.tabs.some(x=>x?.startsWith('To Do'))||!workMetrics.tabs.some(x=>x?.startsWith('In Progress'))||!workMetrics.tabs.some(x=>x?.startsWith('Done')))throw new Error('Work queue must expose To Do, In Progress, and Done.'); if(!workMetrics.hasIncident||!workMetrics.hasChanges||!workMetrics.hasNotes)throw new Error('Work must expose Incidents, Changes, and Notes as separate sections.'); if(workMetrics.hasHorizontalOverflow)throw new Error('Work page has horizontal overflow at the mobile viewport.'); await capture(cdp,'work-mobile');
  await writeFile(`${artifactDir}/metrics.json`,JSON.stringify({url:liveUrl.toString(),alignment,work:workMetrics},null,2)); console.log(`Live visual QA passed. Home delta: ${alignment.delta.toFixed(1)}px; Work queue + Incidents + Changes + Notes present.`);

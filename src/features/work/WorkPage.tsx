  ...
  return (
    <main className="shell work-page still-work-page">
      <header className="still-work-header">
        ...
        <div><p className="section-kicker">Life area</p><h1>Work</h1><p>Schedule, shifts, and earnings in one calm view.</p></div>
      ...
      <section className="still-work-live card">
        <div className="still-work-live-top">
          ...
          <div><small>{activeShift ? (onBreak ? 'On break' : 'Shift in progress') : 'Earned today'}</small><strong className={privacyBlur ? 'private-value is-blurred' : 'private-value'}>{currency(earnedToday, profile.currency)}</strong></div>
        ...
        <div className="still-work-live-meta"><span>{durationLabel(activeShift ? workedHours(activeShift, now) : workedToday)}</span><span>{activeShift ? 'current shift' : 'worked today'}</span></div>
      ...
      <section className="still-work-summary">
        <article className="card"><small>Next shift</small><strong>{nextShift ? nextShift.date.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' }) : 'Not scheduled'}</strong><span>{nextShift ? `${nextShift.start} – ${nextShift.end}` : 'Add a workday below'}</span></article>
        ...
        <article className="card"><small>This week</small><strong>{durationLabel(weekHours)}</strong><span>{durationLabel(weekOvertime)} overtime</span></article>
      </section>

      <section className="still-work-section">
        <div className="still-work-section-heading"><div><h2>Shifts</h2><p>Add past shifts or correct recorded time.</p></div><button onClick={() => setEditingShift({ value: initialShiftDraft(profile) })} type="button"><Plus size={16} />Add shift</button></div>
        <div className="work-shift-list">
          {shifts.length === 0 ? <div className="card work-shift-empty">Your recorded shifts will appear here.</div> : shifts.slice(0, 12).map((shift) => (
      ...
      <section className="still-work-section" id="work-settings">
        <div className="still-work-section-heading"><div><h2>Schedule & pay</h2><p>Set each weekday, date exceptions, and exact pay dates.</p></div></div>

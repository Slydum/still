...
export function LifeAreaPage() {
  ...
  const total = records.tasks.length
    + records.events.length
    + records.journalEntries.length
    + records.expenses.length
    + records.workShifts.length;

  const sortedTasks = [...records.tasks].sort((a, b) => {
    if (a.completed !== b.completed) return a.completed ? 1 : -1;
    return (a.dueDate ?? '9999-12-31').localeCompare(b.dueDate ?? '9999-12-31') || b.updatedAt - a.updatedAt;
  });
      ...
      {total === 0 ? (
        ...
        <div className="life-area-record-sections">
          {sortedTasks.length > 0 && (
            <section className="life-area-record-section">
              <div className="life-area-section-heading"><CheckSquare size={18} /><h2>{sectionTitle(sortedTasks.length, 'task')}</h2></div>
              <div className="life-area-record-list">
                {sortedTasks.map((task) => (
                  <button className="card life-area-record" key={task.id} onClick={() => openTaskEditor(task.id)} type="button">
                    <span className={`life-area-record-state ${task.completed ? 'is-complete' : ''}`}>{task.completed ? '✓' : ''}</span>
                    <span><strong>{task.title}</strong><small>{task.dueDate ? `Due ${dateLabel(task.dueDate)}` : 'No due date'} · {task.priority} priority</small></span>
                  </button>
                ))}
              </div>
            </section>
          ...
          {sortedEvents.length > 0 && (
            <section className="life-area-record-section">
              <div className="life-area-section-heading"><CalendarDays size={18} /><h2>{sectionTitle(sortedEvents.length, 'event')}</h2></div>
              <div className="life-area-record-list">
                {sortedEvents.map((event) => (
                  <button className="card life-area-record" key={event.id} onClick={() => openEventEditor(event.id)} type="button">
                    <CalendarDays size={18} />
                    <span><strong>{event.title}</strong><small>{dateLabel(event.startDate)}{event.allDay ? ' · All day' : event.startTime ? ` · ${event.startTime}` : ''}</small></span>
                  </button>
                ))}
              </div>
            </section>
          ...
          {sortedJournals.length > 0 && (
            <section className="life-area-record-section">
              <div className="life-area-section-heading"><BookOpen size={18} /><h2>{sectionTitle(sortedJournals.length, 'reflection')}</h2></div>
              <div className="life-area-record-list">
                {sortedJournals.map((entry) => (
                  <button className="card life-area-record" key={entry.id} onClick={() => openJournalEditor(entry.id)} type="button">
                    <BookOpen size={18} />
                    <span><strong>{entry.title || entry.body.slice(0, 72) || 'Reflection'}</strong><small>{dateLabel(entry.entryDate)}</small></span>
                  </button>
                ))}
              </div>
            </section>

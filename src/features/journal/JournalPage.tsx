  ...
  return (
    <main className="shell journal-page">
      <header className="journal-page-header">
        <div>
          <p className="section-kicker">Your inner weather</p>
          <h1>Journal</h1>
      ...
      <section className="journal-history" aria-labelledby="journal-history-title">
        <div className="journal-history-heading">
          <div>
            <p className="section-kicker">Your reflections</p>
            <h2 id="journal-history-title">Journal history</h2>
          </div>
          <span>{entries.length} {entries.length === 1 ? 'entry' : 'entries'}</span>
        </div>

        {groups.length === 0 ? (
          <button className="journal-empty" onClick={() => openJournalEditor()} type="button">
            <BookOpen size={30} />
            <strong>Your first page is open</strong>
            <span>Write a few honest words. They don’t need to be polished.</span>
          </button>
        ) : (
          <div className="journal-groups">
            {groups.map(([dateKey, dateEntries]) => (
              <section className="journal-date-group" key={dateKey}>
                <div className="journal-date-heading">
                  <h3>{dateHeading(dateKey)}</h3>
                  <span>{dateEntries.length} {dateEntries.length === 1 ? 'entry' : 'entries'}</span>
                </div>
                      ...
                      <article className="card journal-entry-card" key={entry.id}>
                        <div className="journal-entry-content">
                          <div className="journal-entry-title-row">
                            <div>
                              <h4>{entry.title || 'Untitled reflection'}</h4>
                              <time dateTime={new Date(entry.createdAt).toISOString()}>
                                {format(new Date(entry.createdAt), 'h:mm a')}
                              </time>
                            </div>
                            {mood && <span className="journal-entry-mood" title={mood.label}>{mood.emoji} {mood.label}</span>}
                          </div>
                          <p className="journal-entry-body">{entry.body}</p>
                          {entry.tags.length > 0 && (
                            <div className="journal-entry-tags" aria-label="Tags">

...
The Supabase write boundary keeps `sync_still_records` as `SECURITY INVOKER` so row-level security remains authoritative. The authenticated API role receives only `SELECT`, a protocol-safe set of `INSERT` columns, a protocol-safe set of `UPDATE` columns, and sequence `USAGE`; broad public-schema privileges such as `DELETE`, `TRUNCATE`, `TRIGGER`, and `REFERENCES` are removed. `server_revision`, ownership/identity columns, and server timestamps are not client-writable. A `BEFORE UPDATE` trigger applies the same `(sync_counter, mutation_id)` ordering rule as the RPC and allocates a fresh server revision for every accepted update. Physical deletion is unavailable to the client role, so synchronized deletion must remain a tombstone. The RPC rejects requests larger than 500 input records; the browser currently sends batches of 250.

Direct own-row Data API writes are therefore not treated as a privileged bypass. They can succeed only when they are equivalent to a valid sync mutation and remain subject to RLS, table constraints, logical-version ordering, and server-managed revisions. Cross-account access is still denied by RLS.
